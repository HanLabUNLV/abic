...
i = 'NMF_RandomForestClassifier.pkl'
pipe = joblib.load(model_dir+i)
W = pipe.named_steps.NMF.transform(X)
H = pipe.named_steps.NMF.components_
ax = sns.heatmap(W)
ax.set_title(i.split('.')[0]+' W Matrix') 
plt.savefig('data/heatmap_w_'+i.split('.')[0]+'.png')
plt.clf()
plt.figure(figsize=(15,36))
ax = sns.heatmap(H.T)
plt.yticks(ticks=range(0,len(fnames)), labels=fnames)
ax.set_title(i.split('.')[0]+' H Matrix')
plt.savefig('data/heatmap_h_'+i.split('.')[0]+'.png')
