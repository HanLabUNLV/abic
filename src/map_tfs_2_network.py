...
gene_tss_file = 'data/gene_tss.subset1.tsv'
with open(gene_tss_file, 'r') as f:
    lines = f.readlines()[1:]
gene_tss = {}
for line in lines:
    chrm, gene, tss = line.strip().split('\t')
    if Path('data/gene_networks_wd/'+gene+'_network.pkl').is_file():
        tss = int(tss)
        gene_tss[gene] = tss


tf_matrix = pd.read_csv('data/enhancer_chipseq_featurematrix.subset1.tsv',sep='\t')
#gene = 'JUNB'
