...
df = pd.read_csv(datadir+'Gasperini2019.bygene.ABC.TF.txt', sep="\t", index_col=0)
#del df[df.columns[0]]
#del df[df.columns[0]]

#chrs = [y for x, y in df.groupby('chrEnhancer')]
df_by_chr = {chr_v: df[df['chr'] == chr_v].copy() for chr_v in df.chr.unique()}

for chromosome in df_by_chr.keys():
  chr_df = df_by_chr[chromosome]
  chr_df['midbin'] = round(chr_df[['TargetGeneTSS']]/1000000)
  chr_df.sort_values(['TargetGeneTSS'], ascending=[True], inplace=True, ignore_index=True)
  chr_df['group'] = 'NA'
  ...
  newdf.to_csv(datadir+'Gasperini2019.bygene.ABC.TF.grouped.txt', sep="\t")
