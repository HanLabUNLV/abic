...
join genenames.txt TSSpos.txt | awk '{print $2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7}' | bedtools sort -i > ${DATADIR}/TSS.bed
...
# generate target.txt
awk -F"\t" '{print $782}' ${DATADIR}/CRISPR.ABC.TF.cobinding.txt > ${DATADIR}/CRISPR.ABC.TF.cobinding.target.txt
#
