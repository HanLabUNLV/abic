...
def group_by_TSS_pos(arg_df):
  arg_df['midbin'] = round(arg_df[['startTSS', 'endTSS']].mean(axis=1)/1000000)
  arg_df = arg_df.sort_values(['startTSS'], ascending=[True])
  arg_df['group'] = 'NA'
...
if __name__ == '__main__':
  ...
  for chromosome in df_by_chr.keys():
    ...
    if 'chrEnhancer' in chr_df.columns:
      new_chr_df = group_by_enhancer_pos(chr_df)
    else:
      new_chr_df = group_by_TSS_pos(chr_df)
