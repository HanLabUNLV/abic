...
#add tsne distance vector

#####reindex columns
####data2 = data2.reindex(columns=data2.columns.tolist() +['t'+str(i) for i in range(16)])
#####iter through genes
####idx_dir = '/data8/han_lab/mhan/abcd/graph.pandas/'
####for gene in set(data2['gene'].tolist()):
####    #find gene index
####    chrm = data2.loc[data2['gene']==gene, 'chr'].tolist()[0]
####    gindxf = pd.read_csv(idx_dir+'vertices_TSS.'+chrm+'.txt',sep='\t')
####    try:
####        gindx = gindxf.loc[gindxf['id']==gene,'index'].values[0]
####    except IndexError:
####        print('ERROR 101: tss node not found for gene:\t' + gene)
####        continue
####    #open enh idx file
####    eidxf = pd.read_csv(idx_dir+'vertices_ep.'+chrm+'.txt', sep='\t')
####    eidxf[['xtra','id']] = eidxf['id'].str.split('|',1, expand=True)

####    #open embedding file
####    embd_dir = '/data8/han_lab/mhan/abcd/stellar/metapath2vec.dim16/' 
####    embd = pd.read_csv(embd_dir+'metapath2vec.'+chrm+'.nodes.txt',sep='\t',index_col=0)
####    #iter through enh to find idx
####    local_gene = data2.loc[data2['gene']==gene]
####    for idx, erow in local_gene.iterrows():
####        eid = erow['chr'] + ':' + str(erow['start']) + '-' + str(erow['stop'])
####        eidx = eidxf.loc[eidxf['id']==eid, 'index'].values[0]

####        #now that we have idx of gene and enh, lookup in tsne embedding using idx
####        try: 
####            evec = embd.iloc[eidx].to_numpy()
####            gvec = embd.iloc[gindx].to_numpy()
####        except IndexError:
####            print('ERROR 102: e/gvec not found for: '+gene)
####            continue 
####        #compute distance in tsne space between e and g, representing a minimal contact mapping to 16d/128d
####        tsne_dist = operator_l1(evec, gvec)
####        data2.loc[idx,['t'+str(i) for i in range(16)]] = tsne_dist

#####normalize all tsne coords for ml pipeline
####for i in ['t'+str(j) for j in range(16)]:
####    data2[i] = normalizer.fit_transform(data2[[i]].values)

#print(data2.loc[:,['t'+str(i) for i in range(16)]].describe())
#values look good, include them in training
...
for est in best_estimators:
    joblib.dump(best_estimators[est]['clf'], 'data/trained_models/no_contact_models/'+est+'.pkl')
